<nav x-data="{ open: false }" class="border-b border-gray-100 bg-sky-950 text-white">
        ...
        <div class="flex justify-between h-16 items-center">
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-white bg-orange-500 hover:bg-orange-600 focus:outline-none transition ease-in-out duration-150">
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden bg-orange-500">
        ...
        <div class="pt-4 pb-1 border-t border-orange-400">
            ...
            <div class="mt-3 space-y-1">
                ...
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <x-responsive-nav-link :href="route('logout')" class="text-white hover:bg-orange-600"
                        onclick="event.preventDefault(); this.closest('form').submit();">
                        {{ __('Log Out') }}
                    </x-responsive-nav-link>
                </form>
