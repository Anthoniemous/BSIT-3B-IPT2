<nav x-data="{ open: false }" class="bg-dark dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700">
        ...
        <div class="flex justify-between h-16">
            ...
            <div class="hidden sm:flex sm:items-center sm:ml-6">
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-gray-200 bg-dark hover:text-gray-300 focus:outline-none transition ease-in-out duration-150">

                            {{-- ✅ Customer Profile Image --}}
                            <img 
                                src="{{ asset(session('customer_image') ?? 'img/default-profile.png') }}" 
                                alt="Profile"
                            ...
                            {{-- ✅ Customer Name --}}
                            <div>{{ session('customer_name') ?? 'Customer' }}</div>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            <x-responsive-nav-link :href="url('shop')" :active="request()->routeIs('shop')">
                {{ __('Shop') }}
            </x-responsive-nav-link>
            <x-responsive-nav-link :href="url('#cart')" :active="request()->is('cart')">
                {{ __('Cart') }}
            </x-responsive-nav-link>
             <x-responsive-nav-link :href="url('#checkout')" :active="request()->is('checkout')">
                {{ __('Checkout') }}
            </x-responsive-nav-link>
        ...
        <div class="pt-4 pb-1 border-t border-gray-200 dark:border-gray-600">
            <div class="px-4">
                <div class="font-medium text-base text-gray-800 dark:text-gray-200">{{ session('customer_name') ?? 'Customer' }}</div>
                <div class="font-medium text-sm text-gray-500">{{ session('customer_email') ?? 'customer@email.com' }}</div>
            </div>
