...
class OrderController extends Controller
{
    ...
    private function syncOrdersToLocal()
    {
        ...
        $this->ensureFolderExists(storage_path("app/papay_activity/$userOrdersFolder"));
        $this->ensureFolderExists(storage_path("app/papay_activity/XML/$userOrdersFolder"));

        // Save JSON
        Storage::disk('papay_activity')->put("$jsonFolder/orders.json", $orders->toJson(JSON_PRETTY_PRINT));

        // Save XML
        $xmlContent = $this->convertToXml($orders, 'orders', 'order');
        Storage::disk('papay_activity')->put("XML/$xmlFolder/orders.xml", $xmlContent);

        // Save each user's orders separately
        foreach ($orders->groupBy('user_id') as $userId => $userOrders) {
            Storage::disk('papay_activity')->put("$userOrdersFolder/user_$userId.json", $userOrders->toJson(JSON_PRETTY_PRINT));
            $xmlUserContent = $this->convertToXml($userOrders, 'orders', 'order');
            Storage::disk('papay_activity')->put("XML/$userOrdersFolder/user_$userId.xml", $xmlUserContent);
        }
    }
