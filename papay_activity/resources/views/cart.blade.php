...
<div class="container">
    <h1 class="page-title">🛍️ Your Vape Cart</h1>
    ...
    @if($cartItems->isEmpty())
        <p class="no-orders">💨 Your cart is empty! Add some vape products to get started.</p>
    @else
            ...
            <table>
                ...
                <tbody>
                    @foreach($cartItems as $item)
                        <tr>
                            <td>{{ $item->product->product_name ?? 'Unknown Vape' }}</td>
                            <td>{{ $item->quantity }}</td>
